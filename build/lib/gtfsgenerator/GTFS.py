...
class GtfsWrite():
    ...
    def remove_dup_lines(self, gtfs_file, in_file, out_path):
        """
    ...
    def combine_files(self, wrkbk_dict, configs):
        ...
        for gtfs_file in gtfs_filelist:
            ...
            for key, value in wrkbk_dict.items(): #iterate across workbooks
                ...
                GtfsWrite.remove_dup_lines(self, gtfs_file, gtfs_tmp, out_path)
                x.write_header(gtfs_file, out_path)
                ftmp = open(os.path.join(out_path, gtfs_tmp), 'r').readlines()
                for line in ftmp:
                    ftmp.write(line)
                ftmp.close()
            fout.close()
